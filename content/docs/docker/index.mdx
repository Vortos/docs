...
│   └── entrypoint.sh             ← boot preflight — runs cache:warmup before the server starts
├── postgres/
│   └── init/
...
Every published Dockerfile includes a custom `entrypoint.sh` that runs `php bin/console cache:warmup` before the server or worker process starts.
