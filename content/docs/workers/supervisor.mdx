...
<Callout type="warn" title="Caught before the box">
`RootlessWorkerCheck` fails [`deploy:doctor`](/docs/deploy/doctor) closed if the active worker config is rootful in the single-image model, so a misconfigured supervisord is caught during preflight rather than crash-looping the worker color at cutover.
</Callout>
