...
| Setting | Value |
|---|---|
| TTL | 60 seconds (configurable via `$ttlSeconds` constructor arg) |
| Rebuild lock TTL | 3000 ms |
| Lock retry | Up to 5 retries with 60–300 ms backoff; falls back to direct DB resolve |
| Payload | JSON-encoded `ResolvedPermissions::toArray()` plus role generation hash |
| Serialization | JSON only — no `serialize()`/`unserialize()` to eliminate PHP object injection risk |
