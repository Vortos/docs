---
title: Doctrine ORM
description: Zero-SQL aggregate persistence via Doctrine ORM — AggregateRoot, OrmStore, #[UsesOrmEntity], OrmUnitOfWork, metadata caching, and schema management.
---
...
`OrmRepositoryCompilerPass` detects `#[UsesOrmEntity]` at compile time and injects the configured store. No manual argument configuration needed.
...
When ORM is detected, the command prints `[ORM]` next to the command name in the output. When only DBAL is installed, the `#[UsesDbalMapper]` / `DbalStore` pattern is generated instead.
