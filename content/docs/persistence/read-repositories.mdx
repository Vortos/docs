...
| Pattern | Read store | When to use |
|---|---|---|
| `#[MongoCollection]` + `MongoStore` | MongoDB | Self-contained document projections, high read throughput, frequent schema changes |
| `DbalReadRepository` (abstract) | PostgreSQL | Complex joins or CTEs, single-database setups, full-text search with `pg_trgm` |
