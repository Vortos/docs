...
Both `DbalWriteRepository` and `OrmWriteRepository` implement the same `WriteRepositoryInterface`. Switching between them requires no changes to application code (command handlers, domain services, or tests that use in-memory repositories).
