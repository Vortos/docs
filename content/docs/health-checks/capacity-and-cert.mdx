...
`CertExpiryProbe` inspects a live TLS connection's certificate and reports based on days remaining until expiry. It declares `ProbeKind::Monitoring`, so it runs only on `/health/monitor` — **never** on `/health/ready`:
...
<Callout type="warn" title="Cert expiry is monitoring, not readiness — and never a substitute for renewal automation">
