...
```php
use Vortos\Ses\Attribute\AsBounceHandler;
...
```php
use Vortos\Ses\Attribute\AsComplaintHandler;
...
The package stores suppressed recipients in `ses_suppression_list` by default. Sending behavior is configured with `onSuppressed()`.

```php
$config->suppression()
    ->tableName('ses_suppression_list')
    ->onSuppressed('throw');
```
