...
`MailerInterface` is the normal application path. When outbox is enabled, it writes to `ses_outbox` and requires an active CommandBus or UnitOfWork transaction.
