...
Yes. Install the package, register `SesPackage`, set the required env vars, and use `MailerInterface`.

## Is The Package Optional?

Yes. Projects that do not install or register `vortos/vortos-ses` do not get SES services, commands, tables, or workers.
...
You can also disable only specific sections with `SesObservabilitySection`.
