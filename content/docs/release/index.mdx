...
A `BuildManifest` has to be recorded before [Deploy](/docs/deploy) can act on a build — `deploy` reads the latest manifest for the environment, so with no manifest it refuses (there is nothing to deploy). `vortos:release:record-manifest` writes that record. It runs in the deploy job, **after** the image is built and pushed and **before** `deploy`, and derives the schema fingerprint from the build's available migrations (not the applied set — the migrations have not run yet at this point).
