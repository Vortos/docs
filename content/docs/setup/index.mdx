...
Docker development files load `.env` first and `.env.local` second. Generated Docker passwords and service URLs should stay in `.env.local`; do not copy them into `.env`.
...
Docker images can still require vendor-specific env such as `POSTGRES_PASSWORD` or `MONGO_INITDB_ROOT_PASSWORD`. Setup generates those for service bootstrap, but framework modules do not read them.
