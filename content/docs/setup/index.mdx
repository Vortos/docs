...
Docker development files load `.env` first and `.env.local` second. Generated secrets and service URLs stay in `.env.local`. `APP_NAME` is the exception — setup writes it to `.env` because it is not a secret and should be the same for all developers on the project.

Setup does not rewrite `config/*.php` files. Those files are override points for application-specific changes. The framework modules provide env-driven defaults first: setup writes `VORTOS_*` driver choices and DSNs to `.env.local`, then the module config objects use those env values unless a project config file overrides them. Without an env choice, local-safe in-memory drivers are used where possible.
...
Generated values — written to `.env` (non-secrets) and `.env.local` (secrets and machine-specific):
...
# Docker service credentials (agnostic — compose maps these to POSTGRES_*/MONGO_* internally)
...
Setup generates passwords and stores them in `.env.local`. It does not ask for credentials during normal first-run setup. Existing secrets are preserved on rerun unless you pass `--regenerate-secrets`.
