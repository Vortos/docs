...
This runs 11 checks (C1–C11) — cron validity, name collisions, command allowlisting, lease driver reachability, migration state, 4-eyes approval coverage, misfire policy safety, catchup bounds, shard config, auto-prune config/liveness, and fire-queue consumer liveness. All checks should show `PASS`. A `FAIL` result includes a remediation hint. See [Running](/docs/scheduler/running#scheduler-doctor) for the full check reference.
