...
description: Dispatch domain events through the Vortos EventBus — automatic stamping, routing, outbox, and direct Kafka production.
---
...
- **Outbox enabled** → `OutboxInterface::store()` is called, writing the event to the `vortos_outbox` database table inside your current transaction. The `OutboxRelayWorker` picks it up asynchronously and produces to Kafka.
...
Whether to use the transactional outbox pattern. Defaults to `true`. The outbox table is configured globally in `VortosMessagingConfig`, not per producer. See [Outbox Pattern](/docs/messaging/outbox) for full details.
...
| Header | Value |
|---|---|
| `event_id` | UuidV7 RFC 4122 string — unique, time-ordered, consistent with outbox and dead letter IDs |
| `correlation_id` | UuidV7 or trace correlation ID from the active distributed trace |
