...
vortos:dlq:replay run 11 → x-vortos-global-replays: 11 > limit (10)
    Hard discard — logged, offset committed, no new DLQ entry
```

The default limit is `10`. Set it higher for messages that are likely to require many manual replay cycles (e.g., during prolonged outages), or lower for events where repeated failure indicates a structural problem that needs code-level intervention.
