...
Filters can be combined — for example, `--transport=user.events --event-class="..."` replays only matching messages within that transport. `--id` always selects exactly one row regardless of other filters.
