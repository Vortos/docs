...
php bin/console vortos:dlq:replay --latest --limit=10 --transport=user.events --dry-run
```

Filters can be combined — for example, `--transport=user.events --event-class="..."` replays only matching messages within that transport. `--id` always selects exactly one row regardless of other filters. `--latest` only affects ordering and works with all other flags.
