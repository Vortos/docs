...
- **Disk-buffered retry on the exporter** — every sink's exporter gets `retry_on_failure` backed by a `file_storage` extension, so a backend blip doesn't drop telemetry; it queues to disk and retries.
