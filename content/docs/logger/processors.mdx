---
title: Processors
description: The four built-in Monolog processors — what they add to every log record, how they are ordered, and how to configure each one.
---
...
Monolog processors run on every log record before it reaches a handler. They add structured fields to the record's `extra` array — data like trace IDs, file locations, service metadata, and request context. Vortos ships four processors and applies them in a fixed order.
...
```
1. IntrospectionProcessor      adds file, line, class, function
2. CorrelationIdProcessor      adds trace_id (when tracing is active)
3. RequestContextProcessor     adds http, user, tenant context
4. StructuredLogProcessor      adds ECS/OTel service fields
5. RedactionProcessor          redacts sensitive values   ← runs last
```

Redaction always runs last — it operates on the complete record including fields added by earlier processors.
...
Redaction catches structured context keys. It does not scan free-text message strings. If you concatenate sensitive data into the message string — `$logger->info("User {$email} logged in")` — it will not be redacted. Always use structured context and static message strings.
</Callout>
