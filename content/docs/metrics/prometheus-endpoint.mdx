...
**Dead letter queue events (alert on any value > 0):**
```promql
increase(vortos_messaging_dead_letter_total[5m]) > 0
```
    ...
    rules:
      ...
      - alert: DeadLetterQueueActivity
        ...
        labels:
          severity: critical
        annotations:
          summary: "Messages moved to dead letter queue — manual intervention required"
```
