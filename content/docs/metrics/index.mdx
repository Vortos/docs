---
title: Metrics
description: Counters, gauges, and histograms across HTTP, CQRS, messaging, cache, and persistence — NoOp by default, Prometheus or StatsD when you need real numbers.
---
...
`vortos-metrics` records runtime performance numbers across every major framework module. NoOp by default — zero overhead until you configure an adapter. When you switch to Prometheus or StatsD, counters, gauges, and histograms start flowing with no code changes required.
...
| `{ns}_messaging_produced_total` | Counter | Messaging |
| `{ns}_messaging_consumed_total` | Counter | Messaging |
| `{ns}_messaging_consume_duration_ms` | Histogram | Messaging |
| `{ns}_messaging_dead_letter_total` | Counter | Messaging |
...
<Cards>
  <Card title="Adapters" href="/docs/metrics/adapters" description="NoOp, Prometheus, StatsD — choosing the right adapter, multi-process storage for FrankenPHP." />
