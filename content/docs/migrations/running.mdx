...
- **Framework module tables** come from SQL stubs published via `vortos:migrate:publish` — the generated classes now carry a `#[DeployPhase(MigrationPhase::Expand)]` attribute (or `Contract` when the SQL is destructive), so they pass `vortos:migrate:analyze` out of the box
